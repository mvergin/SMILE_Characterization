...
BASE_DIR = r"C:\_D\programs\SMILE_Characterization\results"
...
PM400_ADDR = None  # None = auto-detect on VISA bus
SMU_ADDR = None  # None = auto-detect on VISA bus

# ── PM400 (optical power meter) ─────────────────────────────────────
PM_WAVELENGTH = 450.0  # nm — detector sensitivity correction
PM_RANGE = 1e-3  # W  — measurement range upper limit

# ── SMU — VLED (Keithley ch A, drives the micro-LED anode) ──────────
VLED_VOLTAGE = 3.0  # V  — forward bias applied to LED
#      constant throughout all phases
VLED_COMPLIANCE = 0.1  # A  — current compliance limit
VLED_NPLC = 1.0  # integration time in power-line cycles
#      (1 NPLC = 20 ms at 50 Hz)
VLED_RANGE_I = 1e-3  # A  — current measurement range
VLED_HIGHC = False  # high-capacitance mode (ch A only)

# ── SMU — NVLED (Keithley ch B, non-radiative recombination path) ───
NVLED_VOLTAGE_FASTSCAN = 0.0  # V  — NVLED bias during Phase 1 (fast scan).
#      The fast scan measures all pixels at
#      this single NVLED voltage to determine
#      which pixels are alive.
NVLED_COMPLIANCE = 0.1  # A  — current compliance limit
NVLED_NPLC = 1.0  # integration time in power-line cycles
NVLED_RANGE_I = 1e-3  # A  — current measurement range
NVLED_HIGHC = False  # high-capacitance mode (not recommended for ch B)

# ── NVLED sweep (Phase 2 & 3) ──────────────────────────────────────
# The sweep steps NVLED voltage from START to TARGET, measuring at
# each step. All voltages are applied per-pixel: pixel 1 gets all
# voltage steps, then pixel 2, etc.
NVLED_SWEEP_START = 0.0  # V  — first sweep voltage
NVLED_SWEEP_TARGET = -5.0  # V  — last sweep voltage
NVLED_SWEEP_STEP = 0.2  # V  — absolute step size (sign auto-determined)
NVLED_SETTLE_MS = 100.0  # ms — wait time after each voltage change
#      before acquiring data (lets current settle)
...
SWEEP_MODE = "Fast Scan"

# ── Fast Scan settings ──────────────────────────────────────────────
# Used in Phase 1 (always) and Phase 2/3 (if SWEEP_MODE = "Fast Scan").
FAST_SCAN_SETTLE_MS = 2.0  # ms — wait after pixel turn-on before
#      first PM400/SMU reading
FAST_SCAN_N_PTS = 10  # number of PM400 + SMU sample pairs per
# pixel per voltage step. More points = better
# averaging but slower.

# ── Full Transient settings ─────────────────────────────────────────
# Only used when SWEEP_MODE = "Full Transient".
PRE_SETTLE_MS = 100.0  # ms — wait after SMU outputs on, before
#      first pixel (thermal settle)
WINDOW_MS = 50.0  # ms — PM400 array capture window length
...
DARK_SETTLE_MS = 50.0  # ms — wait after blank before dark reading
...
DEAD_THRESHOLD_FRAC = 0.10  # 0.10 = less than 10% of median → dead
...
def _build_nvled_voltages(start, target, step):
    """Build the NVLED voltage sweep list."""
    step = abs(step) * (1 if target > start else -1)
    return np.arange(start, target + step * 0.5, step).tolist()
...
def _make_config(measurement_mode, nvled_sweep, nvled_voltage, **overrides):
    ...
    base = dict(
        ...
        nvled_sweep_step=NVLED_SWEEP_STEP,
        nvled_settle_ms=NVLED_SETTLE_MS,
...
def _run_measurement(
    ...
    # Build voltage list
    if cfg.nvled_sweep:
        start_v = cfg.nvled_voltage
        target_v = cfg.nvled_sweep_target
        step_v = abs(cfg.nvled_sweep_step) * (1 if target_v > start_v else -1)
        nvled_voltages = np.arange(start_v, target_v + step_v * 0.5, step_v).tolist()
    else:
    ...
    data_writer = DataWriter(csv_path, error_callback=lambda msg: log(f"WRITER: {msg}"))
    data_writer.start()
